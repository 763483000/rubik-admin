<template>
  <aside class="ra-control">
    <div v-if="!$store.state.headerFixed && $store.state.controlFixed" class="ra-control__toggle" @click="$store.state.controlOpen = !$store.state.controlOpen">
      <svg class="icon-svg" aria-hidden="true"><use xlink:href="#icon-setting"></use></svg>
    </div>
    <div class="ra-control__inner">
      <div class="ra-control__bd">
        <el-tabs class="ra-tabs ra-tabs--flex" v-model="$store.state.controlTabsActive">
          <el-tab-pane label="Layout" name="layout">
            <dl class="ra-control__setting">
              <dt>Wrapper</dt>
