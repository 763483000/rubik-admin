<template>
  <aside class="re-east">
    <div v-if="!$store.state.headerFixed && $store.state.controlFixed" class="re-east__toggle" @click="$store.state.controlOpen = !$store.state.controlOpen">
      <svg class="icon-svg" aria-hidden="true"><use xlink:href="#icon-setting"></use></svg>
    </div>
    <div class="re-east__inner">
      <div class="re-east__bd">
        <el-tabs class="ra-tabs ra-tabs--flex" v-model="$store.state.controlTabsActive">
          <el-tab-pane label="Layout" name="layout">
            <dl class="re-east__setting">
              <dt>Wrapper</dt>
